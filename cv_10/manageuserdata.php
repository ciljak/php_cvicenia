<!-- ******************************************************************* -->
<!-- PHP "self" code for mangment of user data by admin                  -->
<!-- ******************************************************************* -->
<!-- Vrsion: 1.0        Date: 13.12-13.12.2020 by CDesigner.eu           -->
<!-- ******************************************************************* -->
...
<?php
    ...
    // obtain info about ser to edit from get link
    if(isset($_GET['users_id']) && isset($_GET['username'])){
        // take a data from GET link generated by adminscript
        $users_id_for_work = htmlspecialchars($_GET['users_id']);
        $username_for_work = htmlspecialchars($_GET['username']);

        // obtain user-role of currently editing user (is not same as currently editing admin user that is loged in)
        ...
        $sql = "SELECT * FROM bazaar_user WHERE users_id = "."'$users_id_for_work'"."LIMIT 1"  ;
        if($output = mysqli_query($dbc, $sql)){
            if(mysqli_num_rows($output) > 0){  // if any record obtained from SELECT query
                
                while($row = mysqli_fetch_array($output)){ //next rows outputed in while loop
               
                        $user_role_for_work = $row['user_role'];
                        // degug now comment echo "test user role read from databse $user_role_for_work";
                        $nickname_for_work= $row['nickname'];
	                    $first_name_for_work = $row['first_name'];
	                    $lastname_name_for_work = $row['last_name'];
	                    $addresss_for_work = $row['addresss'];
	                    $city_for_work = $row['city'];
                        $ZIPcode_for_work = $row['ZIPcode'];
                        $email_for_work = $row['email'];
                        $rules_accept_for_work = $row['rules_accept'];
                        $GDPR_accept_for_work = $row['GDPR_accept'];
                ...
                echo "Further data about currently editing user can not be obtained."; // if no records in table
            }
        } else{
	...
	if(filter_has_var(INPUT_POST, 'submit')) {
        ...
        $users_id = $users_id_for_work; // obtained from login user
        $username = $username_for_work ;
              ...
              // city_name='" . $_POST['city_name'] . "' ,email='" . $_POST['email'] . "' WHERE userid='" . $_POST['userid'] . "'");
                           ...
                           if(isset($pass_word1) && isset($pass_word2) && isset($pass_word_old )){ // old and two input for new password are provided
                            if($pass_word1 == $pass_word2){ // new passwords is ok typed 2x the same
                                ...
                                $_username = $username_for_work ;
                                // echo "DEBUG -username $_username";
                                $_users_id = $users_id_for_work;
...
<html>
<head>
	<title> Bazaar - edit user properties  </title>
      ...
      <input type="hidden" name="MAX_FILE_SIZE" value="5242880">
          ...
          <?php // here read data from bazar_user table and prefill input fileds with previeously obtained data from user
					 	$dbc = mysqli_connect(DB_HOST, DB_USER, DB_PW, DB_NAME);
                            ...
                            $_username = $users_id_for_work;
                            $_users_id = $username_for_work ;
							...
							if($output = mysqli_query($dbc, $sql)){
								if(mysqli_num_rows($output) > 0){  // if any record obtained from SELECT query
									...
									echo "No records found."; // if no records in table
								}
							} else{
								echo "ERROR: Could not able to execute $sql. " . mysqli_error($dbc); // if database query problem
							}
            ...
            <div id="frame_gray">
              <label> User ID is only one filed that can not be changed, all other can admin of page edit:</label>
              <label>User ID:</label>
		      <input type="text"  name="users_id" class="form-control" value="<?php echo $users_id_for_work ?>" disabled>
              <br>
            </div>  
            <div id="frame_red"> 
              <label>User name:</label>
		      <input type="text"  name="username" class="form-control" value="<?php echo $username_for_work ?>" >
              <br>
              <label>E-mail:</label>
              <input type="text"  name="email" class="form-control" value="<?php echo $email_for_work?>" >
              <br>
            </div>  
              <br>
              <br>
            ...
            <div id="frame_green">
		      ...
		      <input type="text" onfocus="this.value='<?php echo isset($_POST['nickname']) ? $nickname : ''; ?>'" name="nickname" class="form-control" value="<?php echo isset($_POST['nickname']) ? $nickname : $nickname_for_work; ?>">
              <br>
              <label>First name:</label>
		      <input type="text" onfocus="this.value='<?php echo isset($_POST['first_name']) ? $first_name: ''; ?>'" name="first_name" class="form-control" value="<?php echo isset($_POST['first_name']) ? $first_name : $first_name_for_work; ?>">
              <br>
              <label>Last name:</label>
		      <input type="text" onfocus="this.value='<?php echo isset($_POST['lastname_name']) ? $lastname_name : ''; ?>'" name="lastname_name" class="form-control" value="<?php echo isset($_POST['lastname_name']) ? $lastname_name : $lastname_name_for_work; ?>">
              <br>
              <label>Adress in form - Street Nr.:</label>
		      <input type="text" onfocus="this.value='<?php echo isset($_POST['addresss']) ? $addresss : ''; ?>'" name="addresss" class="form-control" value="<?php echo isset($_POST['addresss']) ? $addresss : $addresss_for_work; ?>">
              <br>
              <label>City:</label>
		      <input type="text" onfocus="this.value='<?php echo isset($_POST['city']) ? $city : ''; ?>'" name="city" class="form-control" value="<?php echo isset($_POST['city']) ? $city : $city_for_work; ?>">
              <br>
              <label>ZIP code in form XXXXX:</label>
              <input type="text" onfocus="this.value='<?php echo isset($_POST['ZIPcode']) ? $ZIPcode : ''; ?>'" name="ZIPcode" class="form-control" value="<?php echo isset($_POST['ZIPcode']) ? $ZIPcode : $ZIPcode_for_work; ?>">
              <br>
            </div> 
            ...
            <div id="frame_red">
              ...
              <br>
              <br>
	      	  <input type="checkbox" name="GDPR_accept" class="form-control" <?php if($GDPR_accept_for_work) { echo "checked"; } ?> >
              <label>* I agree with GDPR regulations</label>
              <br>
              <input type="checkbox" name="rules_accept" class="form-control" <?php if($rules_accept_for_work) { echo "checked"; } ?> >
